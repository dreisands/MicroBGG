...
class SharedNetworking {
    ...
    /**
     Makes a request to the Board Game Geek API for results. This one is for the hotness
     ranking. It will unserialize the XML into an Array of Dicionaries.
     
     - Parameter completion: A closure to run on the converted XML
     
     */
    
    func getHotness(completion:(XMLIndexer?) -> Void) {
        UIApplication.sharedApplication().networkActivityIndicatorVisible = true
        print("Calling api: https://www.boardgamegeek.com/xmlapi2/hot/?type=boardgame")
        
        // Test that we can convert the `String` into an `NSURL` object.  If we can
        // not, then crash the application.
        guard let url = NSURL(string: "https://www.boardgamegeek.com/xmlapi2/hot/?type=boardgame")  else {
